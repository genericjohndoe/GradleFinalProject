...
import com.example.chipslibrary.databinding.DetailedChipViewBinding;
import com.example.chipslibrary.models.ChipInterface;
import com.example.chipslibrary.utils.ColorUtil;
import com.example.chipslibrary.utils.LetterTileProvider;
...
public class DetailedChipView extends RelativeLayout {
    ...
    // xml elements
    private DetailedChipViewBinding bind;
    ...
    private void init(AttributeSet attrs) {
        // inflate layout
        bind = DataBindingUtil.bind(inflate(getContext(), R.layout.detailed_chip_view, this));
    ...
    public void setAvatarIcon(Drawable icon) {
        bind.avatarIcon.setImageDrawable(icon);
    }

    public void setAvatarIcon(Bitmap icon) {
        bind.avatarIcon.setImageBitmap(icon);
    }

    public void setAvatarIcon(Uri icon) {
        bind.avatarIcon.setImageURI(icon);
    }
    ...
    public void setInfo(String info) {
        if(info != null) {
            bind.info.setVisibility(VISIBLE);
            bind.info.setText(info);
        }
        else {
            bind.info.setVisibility(GONE);
        }
    }

    public void setTextColor(ColorStateList color) {
        bind.name.setTextColor(color);
        bind.info.setTextColor(ColorUtil.alpha(color.getDefaultColor(), 150));
    }

    public void setBackGroundcolor(ColorStateList color) {
        mBackgroundColor = color;
        bind.content.getBackground().setColorFilter(color.getDefaultColor(), PorterDuff.Mode.SRC_ATOP);
    }

    public int getBackgroundColor() {
        return mBackgroundColor == null ? R.color.colorAccent/*ContextCompat.getColor(mContext, R.color.colorAccent)*/ : mBackgroundColor.getDefaultColor();
    }

    public void setDeleteIconColor(ColorStateList color) {
        bind.deleteButton.getDrawable().mutate().setColorFilter(color.getDefaultColor(), PorterDuff.Mode.SRC_ATOP);
    }

    public void setOnDeleteClicked(OnClickListener onClickListener) {
        bind.deleteButton.setOnClickListener(onClickListener);
    }

    public void alignLeft() {
        RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) bind.content.getLayoutParams();
        params.leftMargin = 0;
        bind.content.setLayoutParams(params);
    }
